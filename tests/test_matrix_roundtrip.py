        ...
        for fid, fd in fmt_defs.items():
            ...
            for si in range(n_samples):
                ...
                if fd.type is Type.FLOAT:
                    ...
                    got_vals = (_strip_fmt_padding_float(got[si])
                                if is_dot else list(got[si]))
                    np.testing.assert_array_equal(
                        np.float32(got_vals), np.float32(exp),
                        err_msg=f"FMT {fid} r{ri} s{si}")
                elif fd.type is Type.INTEGER:
                    # For Number=. cyvcf2 pads with INT_MISSING sentinel
                    got_vals = (_strip_fmt_padding_int(got[si])
                                if is_dot else [int(x) for x in got[si]])
                    assert ([int(x) for x in got_vals] ==
                            [int(x) for x in exp]), \
